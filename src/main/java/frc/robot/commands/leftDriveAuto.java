...
public class leftDriveAuto extends SequentialCommandGroup{
     public leftDriveAuto(CommandSwerveDrivetrain drivetrain,shooterNeoVortex shooterN,Shooter shooterK, LimelightShooter limelight,Indexer indexer){
        addCommands(new PathPlannerAuto("moveBackAuto"),  new FaceAprilTag(drivetrain, limelight).withTimeout(2),new WaitCommand(1),
        new ParallelCommandGroup(new closeNeo2(shooterN), new shooterLime(shooterK), new IndexSpin(indexer)));
     }
}
